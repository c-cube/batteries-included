...
(** {6 Generations of random values.} *)

val gen_bits  : unit    -> int BatGen.t

val gen_int   : int     -> int BatGen.t

val gen_bool  : unit    -> bool BatGen.t

val gen_float : float   -> float BatGen.t

val gen_int32 : Int32.t -> Int32.t BatGen.t

val gen_int64 : Int64.t -> Int64.t BatGen.t

val gen_nativeint : Nativeint.t -> Nativeint.t BatGen.t

val gen_char : unit -> char BatGen.t

(*val gen_uchar : unit -> UChar.t BatGen.t*)

(** {6 Working with data structures.} *)

val choice_gen : 'a BatGen.t -> 'a
(** [choice e] returns a randomly-chosen element of [e].

    This function only works on finite generations with
    less than 2{^30} elements.*)

val multi_choice_gen : int -> 'a BatGen.t -> 'a BatGen.t
...
val shuffle_gen: 'a BatGen.t -> 'a array
...
module State : sig
  ...
  val char       : t -> char
  (*  val uchar      : t -> UChar.t*)
  val gen_bits  : t -> unit    -> int BatGen.t
  val gen_int   : t -> int     -> int BatGen.t
  val gen_bool  : t -> unit    -> bool BatGen.t
  val gen_float : t -> float   -> float BatGen.t
  val gen_int32 : t -> Int32.t -> Int32.t BatGen.t
  val gen_int64 : t -> Int64.t -> Int64.t BatGen.t
  val gen_nativeint : t -> Nativeint.t -> Nativeint.t BatGen.t
  val gen_char  : t -> unit    -> char BatGen.t
      (*  val gen_uchar : t -> unit    -> UChar.t BatGen.t*)
