...
module type IO_MONAD = sig
  type 'a m
  val bind : 'a m -> ('a -> 'b m) -> 'b m   (** Monadic bind operation *)
  val return : 'a -> 'a m             (** Return simple value *)
  val fail : exn -> 'a m              (** Raise exception within monad *)
  val sync : ('a -> 'b) -> 'a -> 'b   (** Atomic operation *)
end
(** Abstraction over a monadic IO system (Lwt, Async, etc.) *)

module Sequential_IO_Monad : IO_MONAD with type 'a m = 'a
(** Just evaluate sequentially. [fail] is just [raise].
    A lock is used for synchronization. *)

module type S = sig
  module IO_Monad : IO_MONAD
...
(** {6 Functor over IO_MONAD} *)

module Make(M : IO_MONAD) : S with module IO_Monad = M

(** {6 Default implementation} *)

include S with module IO_Monad = Sequential_IO_Monad
